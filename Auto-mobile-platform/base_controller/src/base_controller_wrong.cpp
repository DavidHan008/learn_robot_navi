...
unsigned char d_speed_data[10]={0}; 
...
unsigned char data_length=0x08;  //长度
...
void callback(const geometry_msgs::Twist & cmd_input)//订阅/cmd_vel主题回调函数
{
    ...
    d_speed_data[0]=data_head;
   //添加ID
    d_speed_data[1]=data_ID;
  //添加长度
    d_speed_data[2]=data_length;
   //添加命令
    d_speed_data[3]=data_cmd1;
     d_speed_data[4]=data_cmd2;
  //添加截止位置
     d_speed_data[9]=data_end;




    //将转换好的小车速度分量为左右轮速度
    left_speed_data.d  = linear_temp - 0.5f*angular_temp*D ;//为了显示出来 负数表示左转
    ...
    for(int i=0;i<4;i++)    //将左右轮速度存入数组中发送给串口
    {
       ...
       // std::cout<<"right_speed_data.data[i]:"<<i<<"----"<<(int)right_speed_data.data[i]<<std::endl;
        speed_data[i+4]=left_speed_data.data[i];
    }
   // std::cout<<"right_speed_data end"<<std::endl;
    //在写入串口的左右轮速度数据后加入3e
...
d_speed_data[5]=speed_data[2];
d_speed_data[6]=speed_data[3];
d_speed_data[7]=speed_data[6];
d_speed_data[8]=speed_data[7];



for(int i=0;i<10;++i)
{
std::cout<<std::hex<<(int)d_speed_data[i]<<std::endl;
}
 my_serial.write(d_speed_data,10);
